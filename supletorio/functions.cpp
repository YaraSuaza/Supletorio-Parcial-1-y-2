...
void inicio_usu(map <int,tarifas> &valor,map <int,nivel> &nive){
...
void usuario(map <int,tarifas> &valor,map <int,nivel> &nive,string usuario){
...
archiva esa informacion*/
    ...
    string history;
    while (var){
        cout<<"\nEstas son las tarfias disponibles\n";
        //mostrar_tarifas(nive,tarifas);
        cout<<"\nDigite el numero de nivel en que desea parquear: ";
        cin>>temp;
        if(temp<1 || nive.find(temp)==nive.end())
            cout<<"\nNo hay celdas disponibles en ese nivel\n";
}
}

void iniciar_tarif(map <int,tarifas> &valor){
...
y se la devuelven al programa en el map que se le da por referencia*/
    ...
    string temp="",name,datos=leer_Txt("tarifas.txt");
    tarifas a;
    int j,num,cont[2];
    long long int costo;
    for(unsigned long int i=0;i<datos.length();i++){
        ...
        while(i<datos.length()){
            if(datos[i]==';' || datos[i]=='\n' || i+1==datos.length()){
                if(i+1==datos.length()) temp.push_back(datos[i]);
                if(j==0){
                    valor.insert(pair<int,tarifas>(str2int(temp),a));
                    num=str2int(temp);}
                else if(j==1)
                    name=temp;
                else if(j==2)
...
//        valor[num].iniciar(name,costo,ff);
//        while(ff.size()!=0){
//            it=ff.begin();
//            ff.erase(it->first);}
    }
